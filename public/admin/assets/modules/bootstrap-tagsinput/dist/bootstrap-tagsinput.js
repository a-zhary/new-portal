(function ($) {
    ...
    TagsInput.prototype = {
        ...
        add: function (item, dontPushVal, options) {
            ...
            if (self.items().toString().length + item.length + 1 >
        ...
        remove: function (item, dontPushVal, options) {
            ...
            if (self.objectItems) {
                if (typeof item === 'object')
                    item = $.grep(self.itemsArray, function (other) {
                        return self.options.itemValue(other) ==
            ...
            if (item) {
                ...
                $('.tag', self.$container).
                    filter(
                        function () { return $(this).data('item') === item }).
                    remove()
                $('option', self.$element).
                    filter(
                        function () { return $(this).data('item') === item }).
        ...
        build: function (options) {
            ...
            if (self.options.typeaheadjs) {
                ...
                self.$input.typeahead(typeaheadConfig, typeaheadDatasets).
            ...
            self.$container.on('keypress', 'input', $.proxy(function (event) {
                ...
                var text = $input.val(),
                    maxLengthReached = self.options.maxChars && text.length >=
                ...
                if (self.options.freeInput &&
                    ...
                    if (text.length !== 0) {
                        self.add(maxLengthReached ? text.substr(0,
                            self.options.maxChars) : text)
                        $input.val('')
                    }
    ...
    function keyCombinationInList(keyPressEvent, lookupList) {
        ...
        $.each(lookupList, function (index, keyCombination) {
            if (typeof (keyCombination) === 'number' && keyPressEvent.which ===
                keyCombination) {
                found = true
                return false
            }
