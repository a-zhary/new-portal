...
(function (mod) {
    ...
    CodeMirror.registerHelper('fold', 'brace', function (cm, start) {
        ...
        outer: for (var i = line; i <= lastLine; ++i) {
            ...
            for (; ;) {
                ...
                if (cm.getTokenTypeAt(CodeMirror.Pos(i, pos + 1)) ==
                    tokenType) {
    ...
    CodeMirror.registerHelper('fold', 'import', function (cm, start) {
        function hasImport(line) {
            ...
            if (!/\S/.test(start.string)) start = cm.getTokenAt(
                CodeMirror.Pos(line, start.end + 1))
            if (start.type != 'keyword' || start.string !=
                'import') return null
            // Now find closing semicolon, return its position
            for (var i = line, e = Math.min(cm.lastLine(), line + 10); i <=
            e; ++i) {
        ...
        if (!has || hasImport(startLine - 1) ||
            ((prev = hasImport(startLine - 2)) && prev.end.line == startLine -
                1))
            return null
        for (var end = has.end; ;) {
