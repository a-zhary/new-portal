...
(function (mod) {
    ...
    function findParagraph(cm, pos, options) {
        var startRE = options.paragraphStart ||
            cm.getHelper(pos, 'paragraphStart')
        for (var start = pos.line, first = cm.firstLine(); start >
        first; --start) {
            ...
            if (paras.length) cm.operation(function () {
                for (var i = paras.length - 1; i >= 0; --i)
                    madeChange = madeChange ||
                        wrapRange(cm, Pos(paras[i].from, 0),
