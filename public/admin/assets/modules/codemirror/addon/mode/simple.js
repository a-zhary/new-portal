...
(function (mod) {
    ...
    function toRegex(val, caret) {
        ...
        return new RegExp((caret === false ? '' : '^') + '(?:' + val + ')',
            flags)
    }
        ...
        return function (stream, state) {
            ...
            for (var i = 0; i < curState.length; i++) {
                ...
                if (matches) {
                    ...
                    if (rule.data.mode)
                        enterLocalMode(config, state, rule.data.mode,
                            rule.token)
                    if (rule.data.indent)
                        state.indent.push(
                    ...
                    if (matches.length > 2 && rule.token && typeof rule.token !=
                        'string') {
                        state.pending = []
    ...
    function enterLocalMode(config, state, spec, token) {
        ...
        !pers; p = p.next)
            if (spec.spec ? cmp(spec.spec, p.spec) : spec.mode ==
                p.mode) pers = p
        var mode = pers ? pers.mode : spec.mode ||
            CodeMirror.getMode(config, spec.spec)
    ...
    function indentFunction(states, meta) {
        return function (state, textAfter, line) {
            if (state.local && state.local.mode.indent)
                return state.local.mode.indent(state.localState, textAfter,
                    line)
                ...
                for (var i = 0; i < rules.length; i++) {
                    ...
                    if (rule.data.dedent && rule.data.dedentIfLineStart !==
                        ...
                        if (m && m[0]) {
                            pos--
                            if (rule.next ||
                                rule.push) rules = states[rule.next ||
                            rule.push]
