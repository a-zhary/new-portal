    ...
    CodeMirror.multiplexingMode = function (outer /*, others */) {
        ...
        return {
            ...
            copyState: function (state) {
                return {
                    ...
                    inner: state.innerActive &&
                        CodeMirror.copyState(state.innerActive.mode,
                            state.inner),
                }
            },
            ...
            token: function (stream, state) {
                if (!state.innerActive) {
                    ...
                    for (var i = 0; i < others.length; ++i) {
                        ...
                        if (found == stream.pos) {
                            if (!other.parseDelimiters) stream.match(
                                other.open)
                            state.innerActive = other
                            state.inner = CodeMirror.startState(other.mode,
                                outer.indent
                                    ? outer.indent(state.outer, '')
                                    : 0)
                    ...
                    if (found == stream.pos && !curInner.parseDelimiters) {
                        ...
                        return curInner.delimStyle &&
                            (curInner.delimStyle + ' ' + curInner.delimStyle +
                                '-close')
                    }
            ...
            indent: function (state, textAfter) {
                ...
                return mode.indent(
                    state.innerActive ? state.inner : state.outer, textAfter)
            },
