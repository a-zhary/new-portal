...
(function (mod) {
    ...
    DiffView.prototype = {
        ...
        init: function (pane, orig, options) {
            ...
            ;(this.edit.state.diffViews ||
                (this.edit.state.diffViews = [])).push(this)
            this.orig = CodeMirror(pane, copyObj({
                value: orig,
                readOnly: !this.mv.options.allowEditingOriginals,
            }, copyObj(options)))
            if (this.mv.options.connect == 'align') {
                if (!this.edit.state.trackAlignable) this.edit.state.trackAlignable = new TrackAlignable(
                    this.edit)
            ...
            if (Object.prototype.toString.call(classLocation) !=
                '[object Array]') classLocation = [classLocation]
    ...
    function syncScroll(dv, toOrig) {
        ...
        if (dv.diffOutOfDate) {
            if (dv.lockScroll && dv.needsScrollSync ==
                null) dv.needsScrollSync = toOrig
            return false
        }
        ...
        if (dv.mv.options.connect == 'align') {
            ...
            var targetPos = (offOther.top - halfScreen) + ratio *
            ...
            } else if ((botDist = sInfo.height - sInfo.clientHeight -
                sInfo.top) < halfScreen) {
                var otherInfo = other.getScrollInfo()
                var botDistOther = otherInfo.height - otherInfo.clientHeight -
                    targetPos
                if (botDistOther > botDist && (mix = botDist / halfScreen) < 1)
                    targetPos = targetPos * mix +
    ...
    function updateMarks(editor, diff, state, type, classes) {
        ...
        editor.operation(function () {
            if (state.from == state.to || vp.from - state.to > 20 ||
                ...
                if (vp.to > state.to) {
                    markChanges(editor, diff, type, state.marked, state.to,
                        vp.to, classes)
                    state.to = vp.to
                }
            }
    ...
    function markChanges(editor, diff, type, marks, from, to, classes) {
        ...
        function markChunk(start, end) {
            ...
            for (var i = bfrom; i < bto; ++i)
                marks.push(addClass(editor, i, classes, true, i == start,
                    i == end - 1))
            // When the chunk is empty, make sure a horizontal line shows up
            if (start == end && bfrom == end && bto == end) {
                if (bfrom)
                    marks.push(
                        addClass(editor, bfrom - 1, classes, false, false,
                            true))
                else
                    marks.push(
                        addClass(editor, bfrom, classes, false, true, false))
            }
        }
    ...
    function alignableFor(cm, chunks, isOrig) {
        ...
        for (var i = 0; ; i++) {
            var chunk = chunks[i]
            var chunkStart = !chunk ? 1e9 : isOrig
                ? chunk.origFrom
                : chunk.editFrom
    ...
    function mergeAlignable(result, origAlignable, chunks, setIndex) {
        ...
        outer: for (; ; rI++) {
            ...
            while (chunkI < chunks.length) {
                ...
                diff += (chunk.origTo - chunk.origFrom) -
    ...
    function findAlignedLines(dv, other) {
        ...
        if (other) for (var i = 0, j = 0; i < other.chunks.length; i++) {
            ...
            if (j == alignable.length || alignable[j] != n) alignable.splice(
                j++, 0, n)
        }
    ...
    function drawConnectorsForChunk(dv, chunk, sTopOrig, sTopEdit, w) {
        ...
        if (dv.svg) {
            var topLpx = top
            var topRpx = dv.edit.heightAtLine(chunk.editFrom, 'local', true) -
                sTopEdit
            if (flip) {
                var tmp = topLpx
                topLpx = topRpx
            ...
            var botLpx = dv.orig.heightAtLine(chunk.origTo, 'local', true) -
                sTopOrig
            var botRpx = dv.edit.heightAtLine(chunk.editTo, 'local', true) -
                sTopEdit
            if (flip) {
                var tmp = botLpx
                botLpx = botRpx
            ...
            var curveTop = ' C ' + w / 2 + ' ' + topRpx + ' ' + w / 2 + ' ' +
                topLpx + ' ' + (w + 2) + ' ' + topLpx
            var curveBot = ' C ' + w / 2 + ' ' + botLpx + ' ' + w / 2 + ' ' +
                botRpx + ' -1 ' + botRpx
            attrs(dv.svg.appendChild(document.createElementNS(svgNS, 'path')),
                'd',
                'M -1 ' + topRpx + curveTop + ' L ' + (w + 2) + ' ' + botLpx +
                curveBot + ' z',
                'class', dv.classes.connect)
        }
        if (dv.copyButtons) {
            var copy = dv.copyButtons.appendChild(
                elt('div', dv.type == 'left' ? '\u21dd' : '\u21dc',
                    'CodeMirror-merge-copy'))
            var editOriginals = dv.mv.options.allowEditingOriginals
            copy.title = editOriginals ? 'Push to left' : 'Revert chunk'
            copy.chunk = chunk
            copy.style.top = (chunk.origTo > chunk.origFrom
                    ? top
                    : dv.edit.heightAtLine(chunk.editFrom, 'local') - sTopEdit) +
                'px'

            if (editOriginals) {
                var topReverse = dv.edit.heightAtLine(chunk.editFrom, 'local') -
                    sTopEdit
                var copyReverse = dv.copyButtons.appendChild(
                    elt('div', dv.type == 'right' ? '\u21dd' : '\u21dc',
    ...
    function copyChunk(dv, to, from, chunk) {
        ...
        var origStart = chunk.origTo > from.lastLine()
            ? Pos(chunk.origFrom - 1)
            : Pos(chunk.origFrom, 0)
        var origEnd = Pos(chunk.origTo, 0)
        var editStart = chunk.editTo > to.lastLine()
    ...
    var MergeView = CodeMirror.MergeView = function (node, options) {
        ...
        if (hasLeft) {
            ...
            var leftPane = elt('div', null,
                'CodeMirror-merge-pane CodeMirror-merge-left')
            wrap.push(leftPane)
            wrap.push(buildGap(left))
        }

        var editPane = elt('div', null,
        ...
        var resizeInterval = setInterval(function () {
            for (var p = wrapElt.parentNode; p && p !=
            document.body; p = p.parentNode) {}
            if (!p) {
    ...
    function buildGap(dv) {
        ...
        CodeMirror.on(lock, 'click',
            function () { setScrollLock(dv, !dv.lockScroll) })
    ...
    function endOfLineClean(diff, i) {
        ...
        var next = diff[i + 1][1]
        if ((next.length == 1 && i < diff.length - 2) || next.charCodeAt(0) !=
            10) return false
    ...
    function collapseStretch(size, editors) {
        ...
        for (var i = 0; i < editors.length; i++) {
            ...
            var mark = collapseSingle(editor.cm, editor.line,
                editor.line + size)
            marks.push(mark)
    ...
    function unclearNearChunks(dv, margin, off, clear) {
        for (var i = 0; i < dv.chunks.length; i++) {
            ...
            for (var l = chunk.editFrom - margin; l < chunk.editTo +
            margin; l++) {
                var pos = l + off
    ...
    function collapseIdenticalStretches(mv, margin) {
        ...
        for (var i = 0; i < clear.length; i++) {
            if (clear[i]) {
                var line = i + off
                for (var size = 1; i < clear.length - 1 &&
                clear[i + 1]; i++, size++) {}
                if (size > margin) {
                    ...
                    if (mv.options.onCollapse) mv.options.onCollapse(mv, line,
                        size, mark)
                }
            }
        }
    ...
    function elt(tag, content, className, style) {
        ...
        if (typeof content == 'string') e.appendChild(
            document.createTextNode(content))
        else if (content) for (var i = 0; i <
        content.length; ++i) e.appendChild(content[i])
        return e
    }
    ...
    TrackAlignable.prototype = {
        ...
        hasMarker: function (n) {
            ...
            handle.markedSpans.length; i++)
                if (handle.markedSpans[i].mark.collapsed &&
        ...
        map: function (from, nBefore, nAfter) {
            ...
            for (var i = 0; i < this.alignable.length; i += 2) {
                ...
                if (n == from &&
                    (this.alignable[i + 1] & F_WIDGET_BELOW)) widgetFrom = i
                if (n == to &&
                    (this.alignable[i + 1] & F_WIDGET_BELOW)) widgetTo = i
                if (n <= from) continue
            ...
            if (widgetFrom > -1) {
                ...
                if (flags == F_WIDGET_BELOW) this.alignable.splice(widgetFrom,
                    2)
    ...
    function posMin(a, b) {
        return (a.line - b.line || a.ch - b.ch) < 0
            ? a
            : b
    }
    ...
    function goNearbyDiff(cm, dir) {
        var found = null, views = cm.state.diffViews,
