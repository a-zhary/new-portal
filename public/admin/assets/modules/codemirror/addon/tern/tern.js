...
(function (mod) {
        ...
        request: function (cm, query, c, pos) {
            ...
            this.server.request(request, function (error, data) {
                if (!error && self.options.responseFilter)
                    data = self.options.responseFilter(doc, query, request,
                        error, data)
                c(error, data)
            })
        },
    ...
    function updateArgHints(ts, cm) {
        ...
        for (var line = cm.getCursor().line, e = Math.max(0,
        ...
        if (cache && cache.doc == cm.getDoc() && cmpPos(start, cache.start) ==
            0)
            return showArgHints(ts, cm, argPos)

        ts.request(cm, { type: 'type', preferFunction: true, end: start },
            function (error, data) {
    ...
    function jumpToDef(ts, cm) {
        function inner(varName) {
            ...
            ts.server.request(buildRequest(ts, doc, req),
                function (error, data) {
                    if (error) return showError(ts, cm, error)
                    ...
                    if (data.file) {
                        ...
                        if (localDoc &&
                            (found = findContext(localDoc.doc, data))) {
                            ts.jumpStack.push({
                                file: doc.name,
        ...
        if (!atInterestingExpression(cm))
            dialog(cm, 'Jump to variable',
                function (name) { if (name) inner(name) })
        else
            inner()
    }
    ...
    function findContext(doc, data) {
        ...
        var start = Pos(startLine,
            (before.length == 1 ? data.start.ch : doc.getLine(
                startLine).length) - before[0].length)

        var text = doc.getLine(startLine).slice(start.ch)
        for (var cur = startLine + 1; cur < doc.lineCount() && text.length <
        data.context.length; ++cur)
            text += '\n' + doc.getLine(cur)
    ...
    function rename(ts, cm) {
        ...
        if (!/\w/.test(token.string)) return showError(ts, cm,
            'Not at a variable')
        dialog(cm, 'New name for ' + token.string, function (newName) {
            ts.request(cm, { type: 'rename', newName: newName, fullDocs: true },
                function (error, data) {
                    if (error) return showError(ts, cm, error)
        ...
        ts.request(cm, { type: 'refs' }, function (error, data) {
            ...
            for (var i = 0; i < data.refs.length; i++) {
                ...
                if (ref.file == name) {
                    ...
                    if (cmpPos(curPos, ref.start) >= 0 &&
                        cmpPos(curPos, ref.end) <= 0)
                        cur = ranges.length - 1
                }
            }
    ...
    function buildRequest(ts, doc, query, pos) {
        ...
        if (doc.changed) {
            if (doc.doc.lineCount() > bigDoc && allowFragments !== false &&
                ...
                if (query.start != null) query.start = Pos(
                    query.start.line - -offsetLines, query.start.ch)
        ...
        for (var name in ts.docs) {
            ...
            if (cur.changed && cur != doc) {
                files.push(
                    { type: 'full', name: cur.name, text: docValue(ts, cur) })
                cur.changed = null
            }
        }
    ...
    function getFragmentAround(data, start, end) {
        ...
        return {
            ...
            text: doc.getRange(from,
                Pos(endLine, end.line == endLine ? null : 0)),
        }
    }
    ...
    function tempTooltip(cm, content, ts) {
        ...
        setTimeout(maybeClear,
            ts.options.hintDelay ? ts.options.hintDelay : 1700)
