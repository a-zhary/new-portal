...
(function (mod) {
    ...
    CodeMirror.defineExtension('lineComment', function (from, to, options) {
        ...
        if (firstLine == null ||
            probablyInsideString(self, from, firstLine)) return
        ...
        self.operation(function () {
            if (options.indent) {
                ...
                for (var i = from.line; i < end; ++i) {
                    ...
                    if (line.slice(0, cut) != baseString) cut = firstNonWS(
                        line)
                    self.replaceRange(baseString + commentString + pad,
                        Pos(i, 0), Pos(i, cut))
                }
            } else {
                for (var i = from.line; i < end; ++i) {
    ...
    CodeMirror.defineExtension('uncomment', function (from, to, options) {
        ...
        var end = Math.min(
            ...
            self.lastLine()), start = Math.min(from.line, end)
        ...
        lineComment: {
            ...
            for (var i = start; i <= end; ++i) {
                ...
                if (found > -1 && !/comment/.test(
                    self.getTokenTypeAt(Pos(i, found + 1)))) found = -1
                if (found == -1 && nonWS.test(line)) break lineComment
                if (found > -1 &&
                    nonWS.test(line.slice(0, found))) break lineComment
                lines.push(line)
            }
            self.operation(function () {
                for (var i = start; i <= end; ++i) {
                    ...
                    if (pos < 0) continue
                    if (line.slice(endPos, endPos + pad.length) ==
                        pad) endPos += pad.length
                    didSomething = true
        ...
        var close = endLine.indexOf(endString,
        ...
        var firstEnd = lastStart == -1 ? -1 : startLine.slice(0, from.ch).
            indexOf(endString, lastStart + startString.length)
        if (lastStart != -1 && firstEnd != -1 && firstEnd + endString.length !=
            from.ch) return false
        // Positions of the first endString after the end of the selection, and the last startString before it.
        firstEnd = endLine.indexOf(endString, to.ch)
        var almostLastStart = endLine.slice(to.ch).
        ...
        self.operation(function () {
            self.replaceRange('', Pos(end, close -
