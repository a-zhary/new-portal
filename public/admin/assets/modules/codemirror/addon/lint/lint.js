...
(function (mod) {
    ...
    function showTooltip(e, content) {
        ...
        function position(e) {
            if (!tt.parentNode) return CodeMirror.off(document, 'mousemove',
                position)
    ...
    function updateLinting(cm, annotationsNotSorted) {
        ...
        for (var line = 0; line < annotations.length; ++line) {
            ...
            for (var i = 0; i < anns.length; ++i) {
                ...
                if (options.formatAnnotation) ann = options.formatAnnotation(
                    ann)
                if (state.hasGutter) tipLabel.appendChild(
            ...
            if (state.hasGutter)
                cm.setGutterMarker(line, GUTTER_ID,
                    makeMarker(tipLabel, maxSeverity, anns.length > 1,
                        state.options.tooltips))
        }
        if (options.onUpdateLinting) options.onUpdateLinting(
            annotationsNotSorted, annotations, cm)
    }
