...
(function (mod) {
    if (typeof exports == 'object' && typeof module == 'object') // CommonJS
        ...
        define(['../../lib/codemirror', './searchcursor', '../dialog/dialog'],
            mod)
    ...
    function dialog(cm, text, shortText, deflt, f) {
        if (cm.openDialog) cm.openDialog(text, f,
            { value: deflt, selectValueOnOpen: true })
    ...
    function doSearch(cm, rev, persistent, immediate) {
        ...
        if (persistent && cm.openDialog) {
            ...
            persistentDialog(cm, queryDialog, q, searchNext,
                function (event, query) {
                    var keyName = CodeMirror.keyName(event)
                    var extra = cm.getOption('extraKeys'),
                        cmd = (extra && extra[keyName]) ||
        ...
        cm.operation(function () {
            ...
            if (!cursor.find(rev)) {
                cursor = getSearchCursor(cm, state.query,
                    rev ? CodeMirror.Pos(cm.lastLine()) : CodeMirror.Pos(
                        cm.firstLine(), 0))
    ...
    function replace(cm, all) {
        ...
        var dialogText = '<span class="CodeMirror-search-label">' +
            (all ? 'Replace all:' : 'Replace:') + '</span>'
        dialog(cm, dialogText + replaceQueryDialog, dialogText, query,
            function (query) {
                if (!query) return
