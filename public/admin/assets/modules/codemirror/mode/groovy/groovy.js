    ...
    CodeMirror.defineMode('groovy', function (config) {
        ...
        function tokenBase(stream, state) {
            ...
            if (keywords.propertyIsEnumerable(cur)) {
                if (blockKeywords.propertyIsEnumerable(
                    cur)) curPunc = 'newstatement'
                else if (standaloneKeywords.propertyIsEnumerable(
                    cur)) curPunc = 'standalone'
                return 'keyword'
            }
            return 'variable'
        }
        ...
        function expectExpression(last, newline) {
            return !last || last == 'operator' || last == '->' ||
                /[\.\[\{\(,;:]/.test(last) ||
                last == 'newstatement' || last == 'keyword' || last ==
                'proplabel' ||
        ...
        return {
            startState: function (basecolumn) {
                return {
                    tokenize: [tokenBase],
                    context: new Context((basecolumn || 0) - config.indentUnit,
                        0, 'top', false),
                    indented: 0,
                    startOfLine: true,
                    lastToken: null,
            ...
            indent: function (state, textAfter) {
                if (!state.tokenize[state.tokenize.length -
                1].isBase) return CodeMirror.Pass
