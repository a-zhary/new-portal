...
(function (mod) {
    ...
    var indentingTags = [
        'template', 'literal', 'msg', 'fallbackmsg', 'let', 'if', 'elseif',
        ...
        return {
            ...
            token: function (stream, state) {
                    ...
                    case 'comment':
                        ...
                        if (!state.scopes) {
                            ...
                            for (var match; (match = paramRe.exec(current));) {
                                state.variables = prepend(state.variables,
                                    match[1])
                            }
                        }
                        return 'comment'
                ...
                if (stream.match(/^\/\*/)) {
                    ...
                    return 'comment'
                } else if (stream.match(stream.sol() ||
                (state.soyState.length && last(state.soyState) != 'literal')
                    ? /^\s*\/\/.*/
                    : /^\s+\/\/.*/)) {
                    return 'comment'
                }
                ...
                switch (last(state.soyState)) {
                    case 'templ-def':
                        if (match = stream.match(/^\.?([\w]+(?!\.[\w]+)*)/)) {
                            state.templates = prepend(state.templates,
                                match[1])
                            state.scopes = prepend(state.scopes,
                                state.variables)
                            state.soyState.pop()
                            return 'def'
                        }
                ...
                if (stream.match(/^\{literal}/)) {
                    ...
                    /^\{([\/@\\]?\w+\??)(?=[\s\}]|\/[/*])/)) {
                    if (match[1] != '/switch')
                        state.indent += (/^(\/|(else|elseif|ifempty|case|fallbackmsg|default)$)/.test(
                                match[1]) && state.tag != 'switch' ? 1 : 2) *
                            config.indentUnit
                    state.tag = match[1]
            ...
            indent: function (state, textAfter) {
                ...
                if (top == 'literal') {
                    if (/^\{\/literal}/.test(
                        textAfter)) indent -= config.indentUnit
                } else {
                    if (/^\s*\{\/(template|deltemplate)\b/.test(
                        textAfter)) return 0
                    if (/^\{(\/|(fallbackmsg|elseif|else|ifempty)\b)/.test(
                        textAfter)) indent -= config.indentUnit
                    if (state.tag != 'switch' && /^\{(case|default)\b/.test(
                        textAfter)) indent -= config.indentUnit
                    if (/^\{\/switch\b/.test(
                        textAfter)) indent -= config.indentUnit
                }
                ...
                if (indent && localState.mode.indent) {
                    indent += localState.mode.indent(localState.state,
                        textAfter)
                }
                return indent
            },
