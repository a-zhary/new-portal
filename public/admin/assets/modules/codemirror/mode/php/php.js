...
(function (mod) {
    ...
    function matchSequence(list, end, escapes) {
        ...
        return function (stream, state) {
            var patterns = list[0]
            for (var i = 0; i < patterns.length; i++) if (stream.match(
                patterns[i][0])) {
        ...
        hooks: {
            ...
            '<': function (stream, state) {
                ...
                if (before = stream.match(/<<\s*/)) {
                    ...
                    stream.eatWhile(/[\w\.]/)
                    var delim = stream.current().
                        slice(before[0].length + (quoted ? 2 : 1))
                    if (quoted) stream.eat(quoted)
                    if (delim) {
                        (state.tokStack || (state.tokStack = [])).push(delim,
                            0)
            ...
            '#': function (stream) {
                while (!stream.eol() &&
                !stream.match('?>', false)) stream.next()
                return 'comment'
            },
            '/': function (stream) {
                if (stream.eat('/')) {
    ...
    CodeMirror.defineMode('php', function (config, parserConfig) {
        ...
        function dispatch(stream, state) {
            ...
            if (!isPHP) {
                ...
                if (openPHP != -1) {
                    if (style == 'string' && (m = cur.match(/[\'\"]$/)) &&
                        !/\?>/.test(cur)) state.pending = m[0]
        ...
        return {
            startState: function () {
                ...
                var php = parserConfig.startOpen ? CodeMirror.startState(
                    phpMode) : null
                return {
                    html: html,
                    php: php,
            ...
            copyState: function (state) {
                var html = state.html,
                    ...
                    php = state.php,
                    phpNew = php && CodeMirror.copyState(phpMode, php), cur
