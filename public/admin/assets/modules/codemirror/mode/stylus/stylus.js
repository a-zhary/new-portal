...
(function (mod) {
    ...
    CodeMirror.defineMode('stylus', function (config) {
        ...
        function wordAsValue(word) {
            ...
            else if (wordLC in valueKeywords || wordLC in
                commonAtoms) override = 'atom'
            else if (wordLC == 'return' || wordLC in
                colorKeywords) override = 'keyword'
        ...
        function startOfLine(stream) {
            return stream.sol() || stream.string.match(
        ...
        states.block = function (type, stream, state) {
            ...
            if (type == 'word') {
                ...
                if (override == 'tag') {
                    ...
                    // tag is an attribute
                    if (stream.string.match(new RegExp(
                        '\\[\\s*' + word + '|' + word + '\\s*\\]'))) {
                        override = 'atom'
                        return 'block'
                    }
                    ...
                    if (tagVariablesRegexp.test(word)) {
                        if ((startOfLine(stream) && stream.string.match(/=/)) ||
                            ...
                            override = 'variable-2'
                            if (wordIsBlock(
                                firstWordOfLine(stream))) return 'block'
                            return pushContext(state, stream, 'block', 0)
                        }
                    }
                ...
                if (word == 'return') return pushContext(state, stream, 'block',
                    0)
        ...
        states.keyframes = function (type, stream, state) {
            if (stream.indentation() == '0' &&
                ((type == '}' && startOfLine(stream)) || type == ']' || type ==
                    'hash'
        ...
        return {
            ...
            indent: function (state, textAfter, line) {
                ...
                if (cx.prev &&
                    (ch == '}' && (cx.type == 'block' || cx.type == 'atBlock' ||
                            cx.type == 'keyframes') ||
                        ch == ')' &&
                    ...
                    if (/@|\$|\d/.test(ch) ||
                        ...
                        if (/\,\s*$/.test(prevLineFirstWord)) {
                            ...
                            indent = lineIndent <= prevLineIndent
                                ? prevLineIndent
    ...
    var hintWords = tagKeywords_.concat(documentTypes_, mediaTypes_,
        mediaFeatures_,
