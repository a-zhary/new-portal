...
(function (mod) {
    ...
    CodeMirror.defineMode('q', function (config) {
        ...
        function tokenBase(stream, state) {
            ...
            if (sol)
                if (c == '/')
                    ...
                    if (stream.eol() || /\s/.test(stream.peek()))
                        return stream.skipToEnd(), /^\\\s*$/.test(
                            stream.current())
                            ? (state.tokenize = tokenCommentToEOF)(stream)
                            : state.tokenize = tokenBase, 'comment'
                    else
            ...
            if (/[A-Za-z]|\./.test(c))
                return stream.eatWhile(/[A-Za-z._\d]/), keywords.test(
                    stream.current()) ? 'keyword' : 'variable'
            if (/[|/&^!+:\\\-*%$=~#;@><\.,?_\']/.test(c))
                return null
            if (/[{}\(\[\]\)]/.test(c))
                return null
        ...
        function tokenLineComment(stream, state) {
            return stream.skipToEnd(), /\/\s*$/.test(stream.current())
        ...
        return {
            ...
            token: function (stream, state) {
                ...
                if (curPunc == '(') pushContext(state, ')', stream.column())
                    ...
                    if (state.context && curPunc ==
                        state.context.type) popContext(state)
