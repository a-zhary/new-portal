...
(function (mod) {
    ...
    CodeMirror.defineMode('python', function (conf, parserConf) {
        ...
        var operators = [
            ...
            /^([-+*/%\/&|^]=?|[<>=]+|\/\/=?|\*\*=?|!=|[~!@])/]
        for (var i = 0; i <
        operators.length; i++) if (!operators[i]) operators.splice(i--, 1)
        ...
        if (py3) {
            ...
            myKeywords = myKeywords.concat(
                ['nonlocal', 'False', 'True', 'None', 'async', 'await'])
        ...
        function tokenLexer(stream, state) {
            ...
            var delimiter_index = current.length == 1
                ? '[({'.indexOf(current)
                : -1
            ...
            if (delimiter_index != -1) {
                if (top(state).type ==
                    current) state.indent = state.scopes.pop().offset -
                    hangingIndent
