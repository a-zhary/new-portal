...
(function (mod) {
    ...
    CodeMirror.defineMode('crystal', function (config) {
        function wordRegExp(words, end) {
            return new RegExp((end ? '' : '^') + '(?:' + words.join('|') + ')' +
                (end ? '$' : '\\b'))
        }
        ...
        function tokenBase(stream, state) {
            ...
            if (stream.match(idents)) {
                ...
                if (stream.eat(':')) {
                    ...
                    if (indentKeywords.test(matched)) {
                        if (!(matched == 'fun' && state.blocks.indexOf('lib') >=
                            0) && !(matched == 'def' && state.lastToken ==
                            'abstract')) {
            ...
            if (stream.eat(':')) {
                if (stream.eat('"')) {
                    return chain(tokenQuote('"', 'atom', false), stream,
                        state)
            ...
            if (matched = stream.match(/^<<-('?)([A-Z]\w*)\1/)) {
                return chain(tokenHereDoc(matched[2], !matched[1]), stream,
                    state)
            }
            ...
            if (matched = stream.match(/[({[]/, false)) {
                matched = matched[0]
                return chain(tokenNest(matched, matching[matched], null),
                    stream, state)
            }
        ...
        function tokenNest(begin, end, style, started) {
            return function (stream, state) {
                if (!started && stream.match(begin)) {
                    state.tokenize[state.tokenize.length - 1] = tokenNest(begin,
                        end, style, true)
                    state.currentIndent += 1
                    return style
                }
        ...
        return {
            ...
            electricInput: wordRegExp(
                dedentPunctualsArray.concat(dedentKeywordsArray), true),
            lineComment: '#',
        }
    })
