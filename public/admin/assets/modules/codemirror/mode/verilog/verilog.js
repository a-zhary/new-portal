...
(function (mod) {
    ...
    CodeMirror.defineMode('verilog', function (config, parserConfig) {
        ...
        function tokenBase(stream, state) {
            ...
            if (hooks[ch] && (style = hooks[ch](stream, state)) !=
                false) return style
        ...
        return {
            ...
            token: function (stream, state) {
                ...
                if (style == 'comment' || style == 'meta' || style ==
                    'variable') return style
                ...
                if (curPunc == ctx.type) {
                    ...
                    while (ctx && ctx.type == 'statement') ctx = popContext(
                        state)
                } else if (curPunc == '{') {
                    ...
                    } else if (curKeyword == 'task' && ctx && ctx.type ==
                        'statement') {
                        // Same thing for task
                    } else {
            ...
            indent: function (state, textAfter) {
                if (state.tokenize != tokenBase && state.tokenize !=
                    null) return CodeMirror.Pass
                if (hooks.indent) {
                ...
                else if (ctx.type == ')' && !closing) return ctx.indented +
                    statementIndentUnit
    ...
    function tlvIdentNext(stream) {
        var match
        return (match = stream.match(tlvIdentMatch, false)) && match[2].length >
            0
    }
            ...
            token: function (stream, state) {
                ...
                if (stream.sol() && !state.tlvInBlockComment) {
                    ...
                    if (state.tlvCodeActive && stream.pos == 0 &&
                        (state.indented == 0) &&
                        (match = stream.match(tlvLineIndentationMatch,
                            false))) {
                ...
                if (state.tlvCodeActive) {
                    ...
                    if (style !== undefined) {
                        ...
                        style = // make this style distinct from the previous one to prevent
                            ...
                            // and style it
                            ' ' +
                            tlvScopeStyle(state, stream.pos - tlvIndentUnit,
                                'indent')
                        ...
                        if (// is identifier prefix
                            ...
                            if (stream.column() == state.indented) {
                                // Begin scope.
                                style += ' ' +
                                    tlvScopeStyle(state, stream.column(),
                                        'scope-ident')
                            }
                        } else {
