...
(function (mod) {
    ...
    function getTagRegexp(tagName, anchored) {
        return new RegExp((anchored ? '^' : '') + '<\/\s*' + tagName + '\s*>',
            'i')
    }
    ...
    function findMatchingMode(tagInfo, tagText) {
        for (var i = 0; i < tagInfo.length; i++) {
            var spec = tagInfo[i]
            if (!spec[0] ||
                spec[1].test(getAttrValue(tagText, spec[0]))) return spec[2]
        }
    }
    ...
    CodeMirror.defineMode('htmlmixed', function (config, parserConfig) {
        ...
        if (configScript) for (var i = configScript.length - 1; i >= 0; i--)
            tags.script.unshift(
                ['type', configScript[i].matches, configScript[i].mode])

        function html(stream, state) {
            var style = htmlMode.token(stream, state.htmlState),
                tag = /\btag\b/.test(style), tagName
            if (tag && !/[<>\s\/]/.test(stream.current()) &&
                ...
                state.token = function (stream, state) {
                    ...
                    return maybeBackup(stream, endTag,
        ...
        return {
            ...
            copyState: function (state) {
                var local
                if (state.localState) {
                    local = CodeMirror.copyState(state.localMode,
                        state.localState)
                }
                return {
