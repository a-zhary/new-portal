    ...
    CodeMirror.defineMode('markdown', function (cmCfg, modeCfg) {
        ...
        function blockNormal(stream, state) {
            ...
            if (state.indentationDiff >= 4 &&
                (prevLineIsIndentedCode || state.prevLine.fencedCodeEnd ||
                ...
                state.localMode = modeCfg.fencedCodeBlockHighlighting &&
                    getMode(match[2])
                if (state.localMode) state.localState = CodeMirror.startState(
                    state.localMode)
        ...
        function htmlBlock(stream, state) {
            ...
            if (!htmlModeMissing) {
                ...
                if ((inner.mode.name == 'xml' && inner.state.tagStart ===
                        null &&
        ...
        function local(stream, state) {
            ...
            if (state.fencedEndRE && state.indentation <= maxFencedEndInd &&
                ...
                if (hasExitedList) return switchBlock(stream, state,
                    state.block)
                return returnType
        ...
        function getType(state) {
            ...
            if (state.formatting) {
                ...
                if (typeof state.formatting ===
                    'string') state.formatting = [state.formatting]

                for (var i = 0; i < state.formatting.length; i++) {
                    styles.push(
                        tokenTypes.formatting + '-' + state.formatting[i])
            ...
            if (state.quote) {
                ...
                if (!modeCfg.maxBlockquoteDepth || modeCfg.maxBlockquoteDepth >=
                    ...
                    styles.push(
                        tokenTypes.quote + '-' + modeCfg.maxBlockquoteDepth)
                }
            }
        ...
        function inlineNormal(stream, state) {
            ...
            if (modeCfg.xml && ch === '<' && stream.match(
                /^(!--|[a-z]+(?:\s+[a-z_:.\-]+(?:\s*=\s*[^ >]+)?)*\s*>)/i,
                false)) {
                var end = stream.string.indexOf('>', stream.pos)
                if (end != -1) {
                    var atts = stream.string.substring(stream.start, end)
                    if (/markdown\s*=\s*('|"){0,1}1('|"){0,1}/.test(
                        atts)) state.md_inside = true
                }
                stream.backUp(1)
            ...
            if (modeCfg.xml && ch === '<' && stream.match(/^\/\w*?>/)) {
                ...
                var len = 1,
                    before = stream.pos == 1 ? ' ' : stream.string.charAt(
                        stream.pos - 2)
                while (len < 3 && stream.eat(ch)) len++
                var after = stream.peek() || ' '
                // See http://spec.commonmark.org/0.27/#emphasis-and-strong-emphasis
                var leftFlanking = !/\s/.test(after) &&
                    (!punctuation.test(after) || /\s/.test(before) ||
                        punctuation.test(before))
                var rightFlanking = !/\s/.test(before) &&
                    (!punctuation.test(before) || /\s/.test(after) ||
                        punctuation.test(after))
                var setEm = null, setStrong = null
                if (len % 2) { // Em
                    if (!state.em && leftFlanking &&
                        (ch === '*' || !rightFlanking ||
                            punctuation.test(before)))
                        setEm = true
                    else if (state.em == ch && rightFlanking &&
                        (ch === '*' || !leftFlanking ||
                            punctuation.test(after)))
                        setEm = false
                }
                if (len > 1) { // Strong
                    if (!state.strong && leftFlanking &&
                        (ch === '*' || !rightFlanking ||
                            punctuation.test(before)))
                        setStrong = true
                    else if (state.strong == ch && rightFlanking &&
                        (ch === '*' || !leftFlanking ||
                            punctuation.test(after)))
                        setStrong = false
                }
                if (setStrong != null || setEm != null) {
                    if (modeCfg.highlightFormatting) state.formatting = setEm ==
                    null ? 'strong' : setStrong == null ? 'em' : 'strong em'
        ...
        var mode = {
            ...
            token: function (stream, state) {
                ...
                if (stream != state.thisLine.stream) {
                    ...
                    if (!state.localState) {
                        state.f = state.block
                        if (state.f != htmlBlock) {
                            var indentation = stream.match(/^\s*/,
                                true)[0].replace(/\t/g, expandedTab).length
            ...
            indent: function (state, textAfter, line) {
                if (state.block == htmlBlock &&
                    htmlMode.indent) return htmlMode.indent(state.htmlState,
                    textAfter, line)
                if (state.localState &&
                    state.localMode.indent) return state.localMode.indent(
                    state.localState, textAfter, line)
