    ...
    CodeMirror.defineMode('smalltalk', function (config) {
        ...
        var next = function (stream, context, state) {
            ...
            if (aChar === '"') {
                ...
                token.name = state.expectVariable ? (keywords.test(
                    stream.current()) ? 'keyword' : 'variable') : null
        ...
        var nextComment = function (stream, context) {
            ...
            return new Token('comment',
                stream.eat('"') ? context.parent : context, true)
        }

        var nextString = function (stream, context) {
            stream.eatWhile(/[^']/)
            return new Token('string',
                stream.eat('\'') ? context.parent : context, false)
        }

        var nextSymbol = function (stream, context) {
            stream.eatWhile(/[^']/)
            return new Token('string-2',
                stream.eat('\'') ? context.parent : context, false)
        }
