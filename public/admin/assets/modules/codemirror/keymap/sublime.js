...
(function (mod) {
    ...
    function findPosSubword(doc, start, dir) {
        ...
        if (dir > 0 && start.ch >= line.length) return doc.clipPos(
            Pos(start.line + 1, 0))
        var state = 'start', type
        for (var pos = start.ch, e = dir < 0 ? 0 : line.length, i = 0; pos !=
        e; pos += dir, i++) {
    ...
    cmds.scrollLineUp = function (cm) {
        ...
        if (!cm.somethingSelected()) {
            var visibleBottomLine = cm.lineAtHeight(
                info.top + info.clientHeight, 'local')
    ...
    cmds.selectNextOccurrence = function (cm) {
        ...
        if (CodeMirror.cmpPos(from, to) == 0) {
            ...
            if (!found ||
                isSelectedRange(cm.listSelections(), cur.from(), cur.to()))
    ...
    function selectBetweenBrackets(cm) {
        var ranges = cm.listSelections(), newRanges = []
        for (var i = 0; i < ranges.length; i++) {
            var range = ranges[i], pos = range.head,
            ...
            for (; ;) {
                ...
                if (closing.ch ==
                    mirror.charAt(mirror.indexOf(opening.ch) + 1)) {
    ...
    cmds.goToBracket = function (cm) {
        cm.extendSelectionsBy(function (range) {
            ...
            if (next && CodeMirror.cmpPos(next.pos, range.head) !=
                0) return next.pos
    ...
    cmds.swapLineUp = function (cm) {
        if (cm.isReadOnly()) return CodeMirror.Pass
        var ranges = cm.listSelections(), linesToMove = [],
            at = cm.firstLine() - 1, newSels = []
        for (var i = 0; i < ranges.length; i++) {
            var range = ranges[i], from = range.from().line - 1,
                to = range.to().line
            newSels.push({
            ...
            else if (linesToMove.length) linesToMove[linesToMove.length -
            1] = to
            at = to
        }
        cm.operation(function () {
    ...
    cmds.swapLineDown = function (cm) {
        ...
        for (var i = ranges.length - 1; i >= 0; i--) {
            var range = ranges[i], from = range.to().line + 1,
                to = range.from().line
    ...
    cmds.joinLines = function (cm) {
        ...
        for (var i = 0; i < ranges.length; i++) {
            ...
            joined.push(
                { start: start, end: end, anchor: !range.empty() && from })
        }
        cm.operation(function () {
            var offset = 0, ranges = []
            for (var i = 0; i < joined.length; i++) {
                ...
                var anchor = obj.anchor &&
                    Pos(obj.anchor.line - offset, obj.anchor.ch), head
                for (var line = obj.start; line <= obj.end; line++) {
                    var actual = line - offset
                    if (line == obj.end) head = Pos(actual,
                        cm.getLine(actual).length + 1)
        ...
        for (var i = 0; i < ranges.length; i++) {
            ...
            if (j == found.length)
                marks.push(cm.markText(from, to,
                    { sublimeBookmark: true, clearWhenEmpty: false }))
        }
    }
            ...
            for (var i = cursors.length - 1; i >= 0; i--) {
                ...
                if (toStartOfLine && !/\S/.test(toStartOfLine) && column %
                    ...
                    var prevIndent = new Pos(cursor.line,
                        CodeMirror.findColumn(toStartOfLine,
                            column - indentUnit, indentUnit))
    ...
    cmds.delLineRight = function (cm) {
        cm.operation(function () {
            var ranges = cm.listSelections()
            for (var i = ranges.length - 1; i >= 0; i--)
                cm.replaceRange('', ranges[i].anchor, Pos(ranges[i].to().line),
                    '+delete')
            cm.scrollIntoView()
        })
    }
