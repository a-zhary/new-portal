...
(function (mod) {
    ...
    var Vim = function () {
        ...
        function fatCursorMarks(cm) {
            ...
            for (var i = 0; i < ranges.length; i++) {
                ...
                if (range.empty()) {
                    if (range.anchor.ch <
                        cm.getLine(range.anchor.line).length) {
                        result.push(cm.markText(range.anchor,
                        ...
                        result.push(
                            cm.setBookmark(range.anchor, { widget: widget }))
                    }
                }
            }
            return result
        ...
        function cmKeyToVimKey(key) {
            ...
            if (pieces.length == 1 && pieces[0].length == 1) {
                ...
                return false
            } else if (pieces.length == 2 && pieces[0] == 'Shift' &&
                lastPiece.length == 1) {
            ...
            for (var i = 0; i < pieces.length; i++) {
                var piece = pieces[i]
                if (piece in
                    modifiers) { pieces[i] = modifiers[piece] } else { hasCharacter = true }
        ...
        function getOption(name, cm, cfg) {
            ...
            if (option.callback) {
                ...
                var local = (scope !== 'global') &&
                    (cm && cm.state.vim.options[name])
                return (local || (scope !== 'local') && option || {}).value
            }
        }

        defineOption('filetype', undefined, 'string', ['ft'],
            function (name, cm) {
        ...
        var vimApi = {
            ...
            defineEx: function (name, prefix, func) {
                if (!prefix) {
                    ...
                    throw new Error(
                        '(Vim.defineEx) "' + prefix + '" is not a prefix of "' +
            ...
            findKey: function (cm, key, origin) {
                ...
                function handleKeyInsertMode() {
                    ...
                    var match = commandDispatcher.matchCommand(keys,
                    ...
                    if (keysAreChars) {
                        ...
                        for (var i = 0; i < selections.length; i++) {
                            ...
                            cm.replaceRange('',
                                offsetCursor(here, 0, -(keys.length - 1)), here,
                                '+input')
                        }
                ...
                function handleKeyNonInsertMode() {
                    ...
                    var match = commandDispatcher.matchCommand(
                        keysMatcher[2] || keysMatcher[1], defaultKeymap,
                        vim.inputState, context)
                    if (match.type == 'none') {
                        clearInputState(cm)
                        return false
                        ...
                        return cm.operation(function () {
                            ...
                            try {
                                if (command.type == 'keyToKey') {
                                    ...
                                    commandDispatcher.processCommand(cm, vim,
                                        command)
                                }
                            } catch (e) {
     ...
     */
        ...
        RegisterController.prototype = {
            pushText: function (
                registerName, operator, text, linewise, blockwise) {
        ...
        var commandDispatcher = {
            ...
            processSearch: function (cm, vim, command) {
                ...
                function onPromptClose(query) {
                    ...
                    handleQuery(query, true /** ignoreCase */,
                        true /** smartCase */)
                    var macroModeState = vimGlobalState.macroModeState
                    if (macroModeState.isRecording) {
                ...
                function onPromptKeyUp(e, query, close) {
                    ...
                    if (keyName == 'Up' || keyName == 'Down') {
                        ...
                        query = vimGlobalState.searchHistoryController.nextMatch(
                            query, up) || ''
                        close(query)
                        if (offset &&
                            e.target) e.target.selectionEnd = e.target.selectionStart = Math.min(
                            offset, e.target.value.length)
                    } else {
                        if (keyName != 'Left' && keyName != 'Right' &&
                            keyName != 'Ctrl' && keyName != 'Alt' && keyName !=
                            'Shift')
                            vimGlobalState.searchHistoryController.reset()
                    }
                    var parsedQuery
                    ...
                    if (parsedQuery) {
                        cm.scrollIntoView(findNext(cm, !forward, parsedQuery),
                            30)
                    } else {
                        clearSearchHighlight(cm)
                        cm.scrollTo(originalScrollPos.left,
                            originalScrollPos.top)
                    }
                }

                function onPromptKeyDown(e, query, close) {
                    var keyName = CodeMirror.keyName(e)
                    if (keyName == 'Esc' || keyName == 'Ctrl-C' || keyName ==
                        'Ctrl-[' ||
            ...
            processEx: function (cm, vim, command) {
                ...
                function onPromptKeyDown(e, input, close) {
                    ...
                    if (keyName == 'Up' || keyName == 'Down') {
                        ...
                        up = keyName == 'Up' ? true : false
                        offset = e.target ? e.target.selectionEnd : 0
                        input = vimGlobalState.exCommandHistoryController.nextMatch(
                            input, up) || ''
                        close(input)
                        if (offset &&
                            e.target) e.target.selectionEnd = e.target.selectionStart = Math.min(
                ...
                if (command.type == 'keyToEx') {
                    ...
                    exCommandDispatcher.processCommand(cm,
                        command.exArgs.input)
                } else {
                    if (vim.visualMode) {
                        showPrompt(cm, {
                            onClose: onPromptClose,
            ...
            evalInput: function (cm, vim) {
                ...
                var origHead = copyCursor(vim.visualMode
                ...
                if (operator) {
                    if (operatorArgs.lastSel) {
                        ...
                        var chOffset = Math.abs(
                            lastSel.head.ch - lastSel.anchor.ch)
                        if (lastSel.visualLine) {
                            // Linewise Visual mode: The same number of lines.
                            newHead = Pos(oldAnchor.line + lineOffset,
                                oldAnchor.ch)
                    ...
                    if (vim.visualMode) {
                        ...
                        if (linewise) {
                            ...
                            if (mode == 'block') {
                                ...
                                for (var i = 0; i < ranges.length; i++) {
                                    ranges[i].head.ch = lineLength(cm,
                                        ranges[i].head.line)
                                }
                            } else if (mode == 'line') {
                                ranges[0].head = Pos(ranges[0].head.line + 1,
                                    0)
                            }
                        }
                    } else {
        ...
        var motions = {
                ...
                moveToBottomLine: function (cm, _head, motionArgs) {
                    var line = getUserVisibleLines(cm).bottom - motionArgs.repeat +
                        1
                    return Pos(line,
                        findFirstNonWhiteSpaceCharacter(cm.getLine(line)))
                },
                ...
                moveToOtherHighlightedEnd: function (cm, _head, motionArgs, vim) {
                    if (vim.visualBlock && motionArgs.sameLine) {
                        var sel = vim.sel
                        return [
                            clipCursorToContent(cm,
                                Pos(sel.anchor.line, sel.head.ch)),
                            clipCursorToContent(cm,
                                Pos(sel.head.line, sel.anchor.ch)),
                        ]
                    } else {
                    ...
                    for (var i = 0; i < motionArgs.repeat; i++) {
                        ...
                        for (var key in vim.marks) {
                            ...
                            var isWrongDirection = (motionArgs.forward) ?
                                cursorIsBefore(mark, cursor) : cursorIsBefore(
                                    cursor, mark)
                ...
                moveByDisplayLines: function (cm, head, motionArgs, vim) {
                    ...
                    var repeat = motionArgs.repeat
                    var res = cm.findPosV(cur,
                        (motionArgs.forward ? repeat : -repeat), 'line',
                        vim.lastHSPos)
                    if (res.hitSide) {
                ...
                moveByPage: function (cm, head, motionArgs) {
                    ...
                    var curStart = head
                    var repeat = motionArgs.repeat
                    return cm.findPosV(curStart,
                        (motionArgs.forward ? repeat : -repeat), 'page')
                },
                ...
                moveByScroll: function (cm, head, motionArgs, vim) {
                    ...
                    var repeat = motionArgs.repeat
                    if (!repeat) {
                        repeat = scrollbox.clientHeight /
                            (2 * cm.defaultTextHeight())
                    }
                    var orig = cm.charCoords(head, 'local')
                    motionArgs.repeat = repeat
                    var curEnd = motions.moveByDisplayLines(cm, head, motionArgs,
                        vim)
                    if (!curEnd) {
                        return null
                    }
                ...
                moveToLineOrEdgeOfDocument: function (cm, _head, motionArgs) {
                    var lineNum = motionArgs.forward
                        ? cm.lastLine()
                        : cm.firstLine()
                ...
                textObjectManipulation: function (cm, head, motionArgs, vim) {
                    ...
                    if (mirroredPairs[character]) {
                        ...
                        tmp = expandWordUnderCursor(cm, inclusive,
                            true /** forward */,
                            true /** bigWord */)
                    } else if (character === 'w') {
                        tmp = expandWordUnderCursor(cm, inclusive,
                        ...
                        tmp = findParagraph(cm, head, motionArgs.repeat, 0,
                            inclusive)
                        motionArgs.linewise = true
                        if (vim.visualMode) {
                ...
                repeatLastCharacterSearch: function (cm, head, motionArgs) {
                    ...
                    var increment = (lastSearch.increment ? 1 : 0) *
                        (forward ? -1 : 1)
        ...
        var operators = {
            change: function (cm, args, ranges) {
                ...
                if (!vim.visualMode) {
                    ...
                    var prevLineEnd = new Pos(anchor.line - 1,
                        Number.MAX_VALUE)
        ...
        var actions = {
            ...
            scroll: function (cm, actionArgs, vim) {
                ...
                if (actionArgs.forward) {
                    ...
                    if (newBottom < cursorCoords.bottom) {
                        cursor.line -= (cursorCoords.bottom - newBottom) /
                            lineHeight
            ...
            enterMacroRecordMode: function (cm, actionArgs) {
                var macroModeState = vimGlobalState.macroModeState
                var registerName = actionArgs.selectedCharacter
                if (vimGlobalState.registerController.isValidRegister(
                    registerName)) {
            ...
            toggleVisualMode: function (cm, actionArgs, vim) {
                ...
                if (!vim.visualMode) {
                    ...
                    CodeMirror.signal(cm, 'vim-mode-change', {
                        mode: 'visual',
                        subMode: vim.visualLine ? 'linewise' : vim.visualBlock
                            ? 'blockwise'
                            : '',
                    })
                    updateCmSelection(cm)
            ...
            paste: function (cm, actionArgs, vim) {
                ...
                if (actionArgs.matchIndent) {
                    ...
                    var text = chompedText.replace(/^\s*/gm, function (wspace) {
                        var newIndent = indent +
                            (whitespaceLength(wspace) - firstIndent)
                        if (newIndent < 0) {
                            return ''
                ...
                if (vim.visualMode) {
                    ...
                    var selections = cm.listSelections()
                    var emptyStrings = new Array(selections.length).join('1').
                        split('1')
                    ...
                    if (lastSelectionCurEnd) {
                        vim.lastSelection.headMark = cm.setBookmark(
                            lastSelectionCurEnd)
                    }
                    if (linewise) {
                        curPosFinal.ch = 0
                    ...
                    if (blockwise) {
                        ...
                        cm.setCursor(cur)
                        selectBlock(cm,
                            Pos(cur.line + text.length - 1, cur.ch))
                        cm.replaceSelections(text)
                        curPosFinal = cur
                    } else {
                ...
                if (replaceWith == '\n') {
                    ...
                    if (vim.visualBlock) {
                        ...
                        var spaces = new Array(
                            cm.getOption('tabSize') + 1).join(' ')
                        replaceWithStr = cm.getSelection()
                        replaceWithStr = replaceWithStr.replace(/\t/g, spaces).
                            replace(/[^\n]/g, replaceWith).
                            split('\n')
            ...
            incrementNumberToken: function (cm, actionArgs) {
                ...
                if (match) {
                    ...
                    var number = parseInt(match[1] + digits, base) +
                        (increment * actionArgs.repeat)
                    numberStr = number.toString(base)
                    var zeroPadding = baseStr ? new Array(
                        digits.length - numberStr.length + 1 +
                        match[1].length).join('0') : ''
     ...
     */
        ...
        function clipCursorToContent(cm, cur, includeLineBreak) {
            var line = Math.min(Math.max(cm.firstLine(), cur.line),
                cm.lastLine())
        ...
        function getSelectedAreaRange(cm, vim) {
            ...
            var getLastSelectedAreaRange = function () {
                ...
                if (block) {
                    ...
                    for (var i = selectionStart.line; i <
                    selectionEnd.line; i++) {
        ...
        function makeCmSelection(cm, sel, mode, exclusive) {
            ...
            if (mode == 'char') {
                var headOffset = !exclusive &&
                !cursorIsBefore(sel.head, sel.anchor) ? 1 : 0
        ...
        function expandWordUnderCursor(
            ...
            if (inclusive) {
                ...
                var wordEnd = end
                while (/\s/.test(line.charAt(end)) && end <
                line.length) { end++ }
                if (wordEnd == end) {
                    var wordStart = start
                    while (/\s/.test(line.charAt(start - 1)) && start >
                    0) { start-- }
     ...
     */
        ...
        function moveToWord(cm, cur, repeat, forward, wordEnd, bigWord) {
            ...
            if (forward && !wordEnd) {
                // w
                if (!shortCircuit &&
                    (firstWord.from != curStart.ch || firstWord.line !=
                        curStart.line)) {
                ...
                // ge
                if (!shortCircuit &&
                    (firstWord.to != curStart.ch || firstWord.line !=
                        curStart.line)) {
                    // We did not start in the middle of a word. Discard the extra word at the end.
                    lastWord = words.pop()
                }
        ...
        function findParagraph(cm, head, repeat, dir, inclusive) {
            ...
            if (i > max &&
                !startState) { startState = true } else { inclusive = false }
            for (i = line; i > min; i--) {
        ...
        function splitBySlash(argString) {
            ...
            for (var i = 0; i < slashes.length; i++) {
                if (typeof slashes[i] == 'number')
                    tokens.push(
                        argString.substring(slashes[i] + 1, slashes[i + 1]))
            }
            return tokens
        }
                ...
                for (var i = 0; i < repeat; i++) {
                    ...
                    if (!found) {
                        ...
                        cursor = cm.getSearchCursor(query,
                            (prev) ? Pos(cm.lastLine()) : Pos(cm.firstLine(),
                                0))
                        if (!cursor.find(prev)) {
                            return
                        }
        ...
        function getMarkPos(cm, vim, markName) {
            if (markName == '\'') {
                ...
                if (cm.doc.history.lastModTime == 0) {
                    ...
                    var changeHistory = cm.doc.history.done.filter(
                        function (el) {
                            if (el.changes !== undefined) { return el }
                        })
        ...
        var exCommands = {
            ...
            set: function (cm, params) {
                ...
                if (!optionIsBoolean && value === undefined || forceGet) {
                    ...
                    if (oldValue instanceof Error) {
                        ...
                        showConfirm(cm,
                            ' ' + (oldValue ? '' : 'no') + optionName)
                    } else {
                        showConfirm(cm, '  ' + optionName + '=' + oldValue)
                    }
                } else {
                    var setOptionReturn = setOption(optionName, value, cm,
                        setCfg)
            ...
            sort: function (cm, params) {
                ...
                function parseArgs() {
                    if (params.argString) {
                        var args = new CodeMirror.StringStream(
                            params.argString)
                        ...
                        if (!opts &&
                            !args.eol()) { return 'Invalid arguments' }
                        if (opts[1]) {
                            ...
                            if (decimal + hex + octal >
                                1) { return 'Invalid arguments' }
                ...
                var radix = (number == 'decimal') ? 10 : (number == 'hex')
                    ? 16
                    : (number == 'octal') ? 8 : null
                ...
                text = (!reverse) ? textPart.concat(numPart) : numPart.concat(
                    textPart)
            ...
            global: function (cm, params) {
                ...
                var lineStart = (params.line !== undefined)
                    ? params.line
                    : cm.firstLine()
                var lineEnd = params.lineEnd || params.line || cm.lastLine()
            ...
            substitute: function (cm, params) {
                ...
                doReplace(cm, confirm, global, lineStart, lineEnd, cursor,
                    query, replacePart, params.callback)
            },
        ...
        function doReplace(cm, confirm, global, lineStart, lineEnd,
            ...
            function next() {
                ...
                isInRange(searchCursor.from(), lineStart, lineEnd)) {
                    if (!global && lastPos && searchCursor.from().line ==
                        lastPos.line) {
                        continue
                    }
        ...
        function executeMacroRegister(cm, vim, macroModeState, registerName) {
            ...
            macroModeState.replaySearchQueries = register.searchQueries.slice(
                0)
        ...
        function logInsertModeChange(macroModeState) {
            ...
            if (register && register.pushInsertModeChanges) {
                register.pushInsertModeChanges(
        ...
        function onChange(cm, changeObj) {
            ...
            if (!macroModeState.isPlaying) {
                while (changeObj) {
                    ...
                    if (changeObj.origin == '+input' || changeObj.origin ==
                        'paste'
                        || changeObj.origin ===
                        undefined /* only in testing */) {
