...
(function (mod) {
    ...
    function kill(cm, from, to, ring, text) {
        ...
        if (ring == 'grow' && lastKill && lastKill.cm == cm &&
            posEq(from, lastKill.pos) && cm.isClean(lastKill.gen))
            growRingTop(text)
        else if (ring !== false)
            addToRing(text)
    ...
    function bySentence(cm, pos, dir) {
        ...
        for (; ;) {
            ...
            if (!next) { // End/beginning of line reached
                if (line ==
                    (dir < 0 ? cm.firstLine() : cm.lastLine())) return Pos(line,
                    ch)
            ...
            if (sawWord && /[!?.]/.test(next)) return Pos(line,
                ch + (dir > 0 ? 1 : 0))
    ...
    function repeated(cmd) {
        var f = typeof cmd == 'string'
            ? function (cm) { cm.execCommand(cmd) }
            : cmd
        return function (cm) {
    ...
    function getInput(cm, msg, f) {
        if (cm.openDialog)
            cm.openDialog(
                msg + ': <input type="text" style="width: 10em"/>', f,
                { bottom: true })
        else
            f(prompt(msg, ''))
    }
    ...
    var keyMap = CodeMirror.keyMap.emacs = CodeMirror.normalizeKeyMap({
        ...
        'Alt-Space': function (cm) {
            var pos = cm.getCursor(), from = pos.ch, to = pos.ch,
        ...
        'Alt-G G': function (cm) {
            ...
            getInput(cm, 'Goto line', function (str) {
                var num
                if (str && !isNaN(num = Number(str)) && num == (num | 0) &&
                    num > 0)
                    cm.setCursor(num - 1)
            })
        },
        ...
        'Ctrl-X Tab': function (cm) {
            cm.indentSelection(
                getPrefix(cm, true) || cm.getOption('indentUnit'))
        },
