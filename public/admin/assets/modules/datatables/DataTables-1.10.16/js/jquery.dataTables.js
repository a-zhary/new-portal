...
(function ($, window, document, undefined) {
    ...
    var DataTable = function (options) {
        ...
        this.each(function () {
            ...
            for (i = 0, iLen = allSettings.length; i < iLen; i++) {
                ...
                if (s.nTable == this || s.nTHead.parentNode == this ||
                    (s.nTFoot && s.nTFoot.parentNode == this)) {
                    var bRetrieve = oInit.bRetrieve !== undefined
                        ? oInit.bRetrieve
                        : defaults.bRetrieve
                    var bDestroy = oInit.bDestroy !== undefined
                        ? oInit.bDestroy
                        : defaults.bDestroy
            ...
            oSettings.oInstance = (_that.length === 1)
                ? _that
                : $this.dataTable()
            ...
            _fnCallbackReg(oSettings, 'aoDrawCallback', oInit.fnDrawCallback,
                'user')
            _fnCallbackReg(oSettings, 'aoServerParams', oInit.fnServerParams,
                'user')
            ...
            _fnApplyColumnDefs(oSettings, oInit.aoColumnDefs, aoColumnsInit,
                function (iCol, oDef) {
            ...
            var loadedInit = function () {
                ...
                _fnCallbackReg(oSettings, 'aoDrawCallback', function () {
                    if (oSettings.bSorted || _fnDataSource(oSettings) ===
                        'ssp' || features.bDeferRender) {
                        _fnSortingClasses(oSettings)
                    }
                }, 'sc')
                ...
                if (tfoot.length === 0 && captions.length > 0 &&
    ...
    var _numToDecimal = function (num, decimalPoint) {
        ...
        if (!_re_dic[decimalPoint]) {
            _re_dic[decimalPoint] = new RegExp(_fnEscapeRegex(decimalPoint),
                'g')
        }
        ...
        $.each(user, function (key, val) {
            ...
            if (hungarianKey !== undefined &&
                ...
                if (hungarianKey.charAt(0) === 'o') {
                    ...
                    _fnCamelToHungarian(src[hungarianKey], user[hungarianKey],
                        force)
                } else {
    ...
    function _fnAddColumn(oSettings, nTh) {
        ...
        var oCol = $.extend({}, DataTable.models.oColumn, oDefaults, {
            ...
            'sTitle': oDefaults.sTitle ? oDefaults.sTitle : nTh
                ? nTh.innerHTML
                : '',
    ...
    function _fnColumnOptions(oSettings, iCol, oOptions) {
        ...
        oCol._bAttrSrc = $.isPlainObject(mDataSrc) && (
            attrTest(mDataSrc.sort) || attrTest(mDataSrc.type) ||
            ...
            for (i = aoColDefs.length - 1; i >= 0; i--) {
                ...
                for (j = 0, jLen = aTargets.length; j < jLen; j++) {
                    if (typeof aTargets[j] === 'number' && aTargets[j] >= 0) {
                        ...
                        /* Integer, basic index */
                        fn(aTargets[j], def)
                    } else if (typeof aTargets[j] === 'number' && aTargets[j] <
                        0) {
    ...
    function _fnGetCellData(settings, rowIdx, colIdx, type) {
        ...
        if ((cellData === rowData || cellData === null) && defaultContent !==
            null && type !== undefined) {
        ...
        for (i = 0, iLen = nTrs.length; i < iLen; i++) {
            ...
            while (nCell) {
                if (nCell.nodeName.toUpperCase() == 'TD' ||
                    ...
                    iColspan = (!iColspan || iColspan === 0 || iColspan === 1)
                        ? 1
                        : iColspan
                    iRowspan = (!iRowspan || iRowspan === 0 || iRowspan === 1)
                        ? 1
                        : iRowspan
    ...
    function _fnAjaxParameters(settings) {
        ...
        for (i = 0; i < columnCount; i++) {
            ...
            dataProp = typeof column.mData == 'function'
                ? 'function'
                : column.mData
    ...
    function _fnFilterComplete(oSettings, oInput, iForce) {
        ...
        if (_fnDataSource(oSettings) != 'ssp') {
            /* Global filter */
            _fnFilter(oSettings, oInput.sSearch, iForce, fnRegex(oInput),
    ...
    function _fnFilterColumn(
        settings, searchStr, colIdx, regex, smart, caseInsensitive) {
        if (searchStr === '') {
            return
        }
    ...
    function _fnScrollDraw(settings) {
        ...
        if (settings.scrollBarVis !== scrollBarVis && settings.scrollBarVis !==
            undefined) {
    ...
    function _fnCalculateColumnWidths(oSettings) {
        ...
        if (ie67 || !userInputs && !scrollX && !scrollY &&
            ...
            tmpTable.append($(oSettings.nTHead).clone()).
        ...
        if ((tableWidthAttr || scrollX) && !oSettings._reszEvt) {
            var bindResize = function () {
                $(window).
                    on('resize.DT-' + oSettings.sInstance,
            ...
            if (formatters === aSort.length) {
                ...
                displayMaster.sort(function (a, b) {
                    ...
                    for (k = 0; k < len; k++) {
                        ...
                        fn = oExtSort[sort.type + '-' + sort.dir] ||
    ...
    function _fnLoadState(settings, oInit, callback) {
        ...
        var loaded = function (s) {
            ...
            var abStateLoad = _fnCallbackFire(settings, 'aoStateLoadParams',
                'stateLoadParams', [settings, s])
        ...
        var state = settings.fnStateLoadCallback.call(settings.oInstance,
            settings, loaded)
    ...
    $.extend(_Api.prototype, {
        ...
        iterator: function (flatten, type, fn, alwaysNew) {
            ...
            for (i = 0, ien = context.length; i < ien; i++) {
                ...
                if (type === 'table') {
                ...
                } else if (type === 'column' || type === 'column-rows' ||
                    type === 'row' || type === 'cell') {
                    ...
                    for (j = 0, jen = items.length; j < jen; j++) {
                        ...
                        if (type === 'cell') {
                            ret = fn.call(apiInst, context[i], item.row,
                                item.column, i, j)
                        } else {
                            ret = fn.call(apiInst, context[i], item, i, j,
                                rows)
                        }
        ...
        if (_pluck(data, '_details').length > 0) {
            ...
            api.on(colvisEvent, function (e, ctx, idx, vis) {
                ...
                for (var i = 0, ien = data.length; i < ien; i++) {
                    ...
                    if (row._details) {
                        row._details.children('td[colspan]').
    ...
    var __setColumnVis = function (settings, column, vis) {
        ...
        if (vis) {
            ...
            var insertBefore = $.inArray(true, _pluck(cols, 'bVisible'),
                column + 1)
    ...
    _api_registerPlural('columns().indexes()', 'column().index()',
        function (type) {
            return this.iterator('column', function (settings, column) {
                return type === 'visible' ?
    ...
    _api_register('destroy()', function (remove) {
        ...
        return this.iterator('table', function (settings) {
            ...
            $('th, td', thead).removeClass(classes.sSortable + ' ' +
                classes.sSortableAsc + ' ' + classes.sSortableDesc + ' ' +
    ...
    function _addNumericSort(decimalPlace) {
        $.each(
            ...
            function (key, fn) {
                ...
                if (key.match(/^html\-/)) {
                    _ext.type.search[key +
                    decimalPlace] = _ext.type.search.html
                }
            },
        )
    }
    ...
    $.extend(true, DataTable.ext.renderer, {
        header: {
            _: function (settings, cell, column, classes) {
                ...
                // listening for
                $(settings.nTable).
                    on('order.dt.DT', function (e, ctx, sorting, columns) {
    ...
    function _fnExternApiFunc(fn) {
        return function () {
            var args = [
                _fnSettingsFromNode(this[DataTable.ext.iApiIndex])].concat(
